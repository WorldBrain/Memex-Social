...
createStorageTestSuite("User management storage", ({ it }) => {
  it(
    "should create a user profile for a new user",
    { withTestUser: true },
  ...
  it(
    "should create and update a user public profile for a new user",
    { withTestUser: true },
    async ({ storage, services, auth }) => {
      const { users } = storage.serverModules;
      const userReference = services.auth.getCurrentUserReference()!;
      ...
      const profileData = {
        ...
        subStackURL: "https://praxis.every.to/people/532856-tiago-forte",
        bio:
      ...
      const retrievedUpdated = await users.getUserPublicProfile(userReference);
      expect(retrievedUpdated).toEqual(expect.objectContaining(profileData));
    }
  );
});
