...
createStorageTestSuite('Content sharing storage', ({ it }) => {
    ...
    it(`should support getting shared list keys map for lists`, async ({
        services,
        storage,
    }) => {
        const { contentSharing } = storage.serverModules
        const listReferenceA = await contentSharing.createSharedList({
            listData: { title: 'My list A' },
            localListId: 55,
            userReference: { type: 'user-reference', id: 1 },
        })
        const listReferenceB = await contentSharing.createSharedList({
            listData: { title: 'My list B' },
            localListId: 56,
            userReference: { type: 'user-reference', id: 2 },
        })

        const {
            keyString: adderKeyStringA,
        } = await storage.serverModules.contentSharing.createListKey({
        ...
        const keysMap = await storage.serverModules.contentSharing.getKeysForLists(
            {
                listReferences: [listReferenceA, listReferenceB],
            },
        )

        expect(keysMap.get(listReferenceA.id as string)).toEqual([
            {
                reference: {
                    id: adderKeyStringA,
                    type: 'shared-list-key-reference',
        ...
        expect(keysMap.get(listReferenceB.id as string)).toEqual([
            {
                reference: {
                    id: readerKeyStringB,
                    type: 'shared-list-key-reference',
