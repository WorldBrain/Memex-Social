...
export type UIElementServices<Wanted extends keyof Services = never> = Pick<Services, UIServices | Wanted>

export abstract class UIElement<Props extends {services: UIElementServices} = {services: UIElementServices}, State = {}, Event extends logic.UIEvent<{}> = logic.UIEvent<{}> > extends Component<Props, State> {
    public logic?: logic.UILogic<State, Event>
    protected styleModule? : string
    protected styleBreakpoints? : {[name : string]: number}
    protected styleClasses? : {[elementName : string] : {[className : string] : (state : State) => boolean}}
    ...
    protected get styles() : {[elementName : string] : string} {
        const styles = this.services.styleRegistry.getStyleModule(this.styleModule || this.elementName)
        ...
        return new Proxy({}, {
            get: (target, key) => {
                ...
                if (this.styleClasses) {
                    for (const [className, condition] of Object.entries(this.styleClasses[key as string] || {})) {
    ...
    async processEvent<EventName extends keyof Event>(eventName : EventName, event : Event[EventName]) {
        if (!this.logic) {
            throw new Error('Tried to process event in UIElement without logic')
        }
        const mutation = await this.logic.processUIEvent(eventName, { state: this.state, event, direct: true })
        if (mutation) {
    ...
    componentDidMount() {
        if (this.logic) {
            this.logic.processUIEvent('init', { state: this.state, event: undefined, optional: true })
            if (this.services) {
                this.services.logicRegistry.registerLogic(this.elementName, {
                    ...
                    triggerOutput: (event : string, ...args : any[]) => (this.props as any)[`on${capitalize(event)}`](...args)
                })
            }
        }
        if (this.styleBreakpoints) {
            this.baseEventHandlers.subscribeTo(this.services.device.events, 'rootResize', () => {
                this.forceUpdate()
            })
        }
    }
    ...
    componentWillUnmount() {
        if (this.logic) {
            this.logic.processUIEvent('cleanup', { state: this.state, event: undefined, optional: true })
    ...
    private getBreakpointClassNames(styles : {[name : string] : string}): string[] {
        if (!this.styleBreakpoints) {
            return []
        }
