...
export const SCENARIOS: ScenarioMap<Targets> = {
    'default': scenario<Targets>(({ step, callModification }) => ({
        ...
        startRoute: { route: 'annotationDetails', params: { id: 'default-annotation' } },
        setup: {
            callModifications: ({ storage }) => [
                callModification({
                    name: 'annotation-loading',
                    object: storage.serverModules.contentSharing, property: 'getAnnotation',
                    modifier: 'block'
                }),
                callModification({
    ...
    'annotation-load-error': scenario<Targets>(({ step, callModification }) => ({
        fixture: 'annotated-list-with-user',
        startRoute: { route: 'annotationDetails', params: { id: 'default-annotation' } },
        setup: {
            callModifications: ({ storage }) => [
