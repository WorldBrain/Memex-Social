...
export function checkAccountCollectionInfoMap(options : {
    ...
    const accountCollections = collectAccountCollections(options.storageRegistry)
    const collectionsWithAccountInfo = new Set(Object.keys(options.accountCollections))
    const collectionsMissingAccountInfo = new Set(Object.keys(accountCollections).filter(
